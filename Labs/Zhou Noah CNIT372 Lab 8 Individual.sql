...
INSERT INTO Customer_New_View (CustName, CompanyName, Address, City, State, PostalCode, Phone, EmailAddr)
VALUES ('John Doe', '', '401 N. Grant St.', 'West Lafayette', 'IN', '47907', '', '');

/* QA3 Non-Query Answer

I am unable to add the "John Doe" customer becuase there are null values (CustomerID) where there are not supposed to be null values (CustomerID).
...
SELECT CustomerID, CustFirstName ||' '|| CustLastName AS CustName, CompanyName, Address, City, State, PostalCode, Phone, EmailAddr
FROM CUSTOMER
UNION ALL
...
INSERT INTO Customer_All (CustomerID, CustName, CompanyName, Address, City, State, PostalCode, Phone, EmailAddr)
VALUES ('C-001', 'John Doe', '', '401 N. Grant St.', 'West Lafayette', 'IN', '47907', '', '');

/* QA5 Query Results

I am unable to add the customer because as the error states, data manipulation is not allowed on this view
...
SELECT CustomerID, CustName, CompanyName, CustTitle, Address, City, State, PostalCode, Fax, Phone, EmailAddr
FROM Customer_New
WHERE State = 'IN';
...
INSERT INTO Indiana_Customer (CustomerID, CustName, CompanyName, Address, City, State, PostalCode, Phone, EmailAddr)
VALUES ('C-001', 'John Doe', '', '401 N. Grant St.', 'West Lafayette', 'IN', '47907', '', '');
...
This time, I was able to insert the new customer into the view. The reason behind this is that all of the columns in the view directly
map to the columns in the underlying Customer_New table.
...
After removing the customer from the Indiana_Customer view, the customer now does not exist in either the Customer_New table or the
Indiana_Customer view.
...
After deleting all the data from the Customer_New table, querying the table shows no rows selected which means the table is now empty.
Issuing a ROLLBACKoperation reverses all changes made in the current transaction, restoring all the rows which can be seen in the second
Customer_New table query which shows all the rows.
...
Although the ROLLBACK operation was issued, the TRUNCATE operation is typically auto-committed which means
it cannot be rolled back unlike a DELETE operation
...
REVOKE SELECT ON CUSTOMER_ALL FROM ZHOU1170;
...
-- Question #A17
/* QA17 Non-Query Answer

For another user such as CNIT372TA to be able to execute my Hello_World procedure, they require EXECUTE privileges.
This can be done through GRANT EXECUTE ON HELLO_WORLD TO CNIT372TA.

Sources:
https://docs.oracle.com/en/database/oracle/oracle-database/21/dbseg/configuring-privilege-and-role-authorization.html#GUID-47EDAC03-04DC-4CFA-9B85-65E9F4FF61BA
...
-------------------------------------------------------------------------------------------------------------------------------- -------------------------------------------------------------------------------------------------------------------------------- -------------------------------------------------------------------------------------------------------------------------------- -------------------------------------------------------------------------------------------------------------------------------- ---------------------------------------- --- --- --- ------------------------ ---
PUBLIC                                                                                                                           SYS                                                                                                                              ZHOU1170                                                                                                                         ZHOU1170                                                                                                                         INHERIT PRIVILEGES                       NO  NO  NO  USER                     NO


The CNIT372TA user does not retain the execute permissions on the HELLO_WORLD procedure.
...
GRANTEE                                                                                                                          OWNER                                                                                                                            TABLE_NAME                                                                                                                       GRANTOR                                                                                                                          PRIVILEGE                                GRA HIE COM TYPE                     INH
-------------------------------------------------------------------------------------------------------------------------------- -------------------------------------------------------------------------------------------------------------------------------- -------------------------------------------------------------------------------------------------------------------------------- -------------------------------------------------------------------------------------------------------------------------------- ---------------------------------------- --- --- --- ------------------------ ---
CNIT372TA                                                                                                                        ZHOU1170                                                                                                                         NUMBER_OF_EMPLOYEES                                                                                                              ZHOU1170                                                                                                                         EXECUTE                                  NO  NO  NO  FUNCTION                 NO
PUBLIC                                                                                                                           SYS                                                                                                                              ZHOU1170                                                                                                                         ZHOU1170                                                                                                                         INHERIT PRIVILEGES                       NO  NO  NO  USER                     NO

In this case, the CNIT372TA user still retains the execution permissions on the NUMBER_OF_EMPLOYEES function.

*/

-- Question #A23
/* QA23 Non-Query Answer

Based on questions A20-A22, recreating a procedure does not remove the permissions on it. The permissions are only removed when the procedure is dropped because then,
all of the permissions are dropped alongside the procedure.
